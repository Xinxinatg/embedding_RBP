...
train_data,train_label=data[:-int(teP)-int(teN)].double(),label[:-int(teP)-int(teN)]
test_data,test_label=data[-int(teP)-int(teN):].double(),label[-int(teP)-int(teN):]
...
LOSS_WEIGHT_POSITIVE = (int(trP)+int(trN)+int(vaP)+int(vaN)) / (2.0 * int(trP)+2.0 * int(vaP)) 
LOSS_WEIGHT_NEGATIVE = (int(trP)+int(trN)+int(vaP)+int(vaN)) / (2.0 * int(trN)+2.0 * int(vaN)) 
...
def main():
  ...
  lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,factor=0.75,patience=10, verbose=True)
